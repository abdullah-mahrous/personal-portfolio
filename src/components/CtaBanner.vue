<template>
    <div class="mt-16 sm:mt-20 mb-12 py-8 sm:py-12 lg:py-16 px-5 sm:px-8 lg:px-12 rounded-lg base-border flex flex-col md:flex-row items-center justify-between gap-8 lg:gap-12 relative overflow-hidden" style="background-color: #111217; background-image: linear-gradient(135deg, rgba(131, 21, 231, 0.18) 0%, rgba(131, 21, 231, 0.05) 100%); box-shadow: 0 0 40px rgba(131, 21, 231, 0.12);">
        ...
        <div class="flex-1 flex justify-center md:justify-end relative z-10">
            <Send class="size-20 md:size-24 text-primary animate-fly-paper-plane opacity-90" />
        </div>
    </div>
</template>
...
.animate-fly-paper-plane {
    animation: flyPaperPlane 2s ease-out forwards;
}
...
.btn-glow::before {
    ...
    right: 0;
    bottom: 0;
    background: radial-gradient(circle at center, rgba(131, 21, 231, 0.4), transparent 70%);
...
.btn-glow {
    box-shadow: 0 0 20px rgba(131, 21, 231, 0.3);
}

.btn-glow:hover {
    box-shadow: 0 0 30px rgba(131, 21, 231, 0.6);
}
</style>
